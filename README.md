...
Ces dépendances sont déclarées en **peerDependencies** dans `@aexae/design-system` et doivent être présentes dans l’application consommatrice.
...
```bash
pnpm add @aexae/design-system
```

Avec **npm** :

```bash
npm install @aexae/design-system
```

Avec **yarn** :

```bash
yarn add @aexae/design-system
```
...
import { SnackbarProvider } from "@aexae/design-system";
import App from "./App";
...
- `useTheme` depuis `@aexae/design-system/styles` pour accéder au thème Comète.
- `useSnackbar` depuis `@aexae/design-system` pour afficher des notifications.
...
Le Storybook est dans le dossier `storybook/` et consomme le package local `@aexae/design-system`.
...
```json
"dependencies": {
  "@aexae/design-system": "file:packages/aexae-design-system-0.0.1.tgz"
}
```
